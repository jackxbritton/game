...
void text_init(Text *text, Font *font, const char *str, GLuint program) {
    ...
    const float scale = 1.0f / 64.0f / 24.0f; // TODO
    ...
    for (int i = 0; i < len; i++) {
        ...
        const float x1 = x + gi->metrics.horiBearingX * scale;
        const float x2 = x + (gi->metrics.horiBearingX + gi->metrics.width) * scale;
        const float y1 = (gi->metrics.horiBearingY - gi->metrics.height) * scale;
        const float y2 = gi->metrics.horiBearingY * scale;
